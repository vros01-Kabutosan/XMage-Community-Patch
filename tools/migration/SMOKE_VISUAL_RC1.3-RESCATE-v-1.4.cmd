...
powershell -NoProfile -ExecutionPolicy Bypass -Command "$ErrorActionPreference='Stop'; $c=Get-ChildItem -LiteralPath '%ACTIVE_ROOT%' -Filter 'mage-client-1.4.61.jar' -File -Recurse | Select-Object -First 1; $s=Get-ChildItem -LiteralPath '%ACTIVE_ROOT%' -Filter 'mage-server-1.4.61.jar' -File -Recurse | Select-Object -First 1; if(!$c -or !$s){exit 1}; $c.Directory.Parent.FullName | Set-Content -Encoding UTF8 '%LOG_DIR%\active_client_root.txt'; $s.Directory.Parent.FullName | Set-Content -Encoding UTF8 '%LOG_DIR%\active_server_root.txt'; $c.FullName | Set-Content -Encoding UTF8 '%LOG_DIR%\active_client_jar.txt'; $s.FullName | Set-Content -Encoding UTF8 '%LOG_DIR%\active_server_jar.txt'" > "%LOG_DIR%\active_discovery.log" 2>&1
