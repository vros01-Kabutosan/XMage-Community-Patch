...
title XMage RC1.3 - RESCATE VISUAL v1.5
...
set "STAMP=%STAMP: =0%"
set "LOG_DIR=%LOG_ROOT%\smoke_trigger_RESCATE_v1.5_%STAMP%"
set "SMOKE_ROOT=J:\mtg\_SMOKE\trigger-RESCATE-v1.5_%STAMP%"
...
call :LOG "START RESCATE VISUAL v1.5"
call :LOG "Script=%~f0"
...
call :LOG "Localizando automaticamente la distribucion activa funcional."
powershell -NoProfile -ExecutionPolicy Bypass -Command "$ErrorActionPreference='Stop'; $c=Get-ChildItem -LiteralPath '%ACTIVE_ROOT%' -Filter 'mage-client-*.jar' -File -Recurse | Where-Object {$_.Name -notmatch 'sources|javadoc|tests' -and $_.Directory.Name -eq 'lib'} | Sort-Object LastWriteTime -Descending | Select-Object -First 1; $s=Get-ChildItem -LiteralPath '%ACTIVE_ROOT%' -Filter 'mage-server-*.jar' -File -Recurse | Where-Object {$_.Name -notmatch 'sources|javadoc|tests' -and $_.Directory.Name -eq 'lib'} | Sort-Object LastWriteTime -Descending | Select-Object -First 1; if(!$c -or !$s){exit 1}; $cc=Get-ChildItem -LiteralPath $c.Directory.FullName -Filter 'mage-common-*.jar' -File | Where-Object {$_.Name -notmatch 'sources|javadoc|tests'} | Select-Object -First 1; $sc=Get-ChildItem -LiteralPath $s.Directory.FullName -Filter 'mage-common-*.jar' -File | Where-Object {$_.Name -notmatch 'sources|javadoc|tests'} | Select-Object -First 1; if(!$cc -or !$sc){exit 2}; $c.Directory.Parent.FullName | Set-Content -Encoding ASCII '%LOG_DIR%\active_client_root.txt'; $s.Directory.Parent.FullName | Set-Content -Encoding ASCII '%LOG_DIR%\active_server_root.txt'; $c.Name | Set-Content -Encoding ASCII '%LOG_DIR%\active_client_jar_name.txt'; $s.Name | Set-Content -Encoding ASCII '%LOG_DIR%\active_server_jar_name.txt'; $cc.Name | Set-Content -Encoding ASCII '%LOG_DIR%\active_client_common_name.txt'; $sc.Name | Set-Content -Encoding ASCII '%LOG_DIR%\active_server_common_name.txt'" > "%LOG_DIR%\active_discovery.log" 2>&1
if errorlevel 1 call :FAIL "No se localizaron automaticamente cliente, servidor y mage-common en la instalacion activa."
set /p "ACTIVE_CLIENT_ROOT="<"%LOG_DIR%\active_client_root.txt"
set /p "ACTIVE_SERVER_ROOT="<"%LOG_DIR%\active_server_root.txt"
set /p "ACTIVE_CLIENT_JAR_NAME="<"%LOG_DIR%\active_client_jar_name.txt"
set /p "ACTIVE_SERVER_JAR_NAME="<"%LOG_DIR%\active_server_jar_name.txt"
set /p "ACTIVE_CLIENT_COMMON_NAME="<"%LOG_DIR%\active_client_common_name.txt"
set /p "ACTIVE_SERVER_COMMON_NAME="<"%LOG_DIR%\active_server_common_name.txt"
call :LOG "Active client=%ACTIVE_CLIENT_ROOT%\lib\%ACTIVE_CLIENT_JAR_NAME%"
call :LOG "Active server=%ACTIVE_SERVER_ROOT%\lib\%ACTIVE_SERVER_JAR_NAME%"
if not exist "%ACTIVE_CLIENT_ROOT%\lib\%ACTIVE_CLIENT_JAR_NAME%" call :FAIL "La raiz activa del cliente no es valida."
if not exist "%ACTIVE_SERVER_ROOT%\lib\%ACTIVE_SERVER_JAR_NAME%" call :FAIL "La raiz activa del servidor no es valida."
if not exist "%ACTIVE_CLIENT_ROOT%\lib\%ACTIVE_CLIENT_COMMON_NAME%" call :FAIL "Falta mage-common en el cliente activo."
if not exist "%ACTIVE_SERVER_ROOT%\lib\%ACTIVE_SERVER_COMMON_NAME%" call :FAIL "Falta mage-common en el servidor activo."
...
if not exist "%CLIENT_ROOT%\lib\%ACTIVE_CLIENT_JAR_NAME%" call :FAIL "El clon no contiene el JAR del cliente."
if not exist "%CLIENT_ROOT%\lib\%ACTIVE_CLIENT_COMMON_NAME%" call :FAIL "El clon del cliente no contiene mage-common."
if not exist "%SERVER_ROOT%\lib\%ACTIVE_SERVER_COMMON_NAME%" call :FAIL "El clon del servidor no contiene mage-common."
if not exist "%SERVER_ROOT%\lib\%ACTIVE_SERVER_JAR_NAME%" call :FAIL "El clon no contiene el JAR del servidor."

copy /Y "%NEW_CLIENT_JAR%" "%CLIENT_ROOT%\lib\%ACTIVE_CLIENT_JAR_NAME%" > "%LOG_DIR%\overlay_client.log" 2>&1
if errorlevel 1 call :FAIL "No se pudo aplicar el JAR nuevo del cliente."
copy /Y "%NEW_COMMON_JAR%" "%CLIENT_ROOT%\lib\%ACTIVE_CLIENT_COMMON_NAME%" > "%LOG_DIR%\overlay_common_client.log" 2>&1
if errorlevel 1 call :FAIL "No se pudo aplicar mage-common al cliente."
copy /Y "%NEW_COMMON_JAR%" "%SERVER_ROOT%\lib\%ACTIVE_SERVER_COMMON_NAME%" > "%LOG_DIR%\overlay_common_server.log" 2>&1
...
powershell -NoProfile -ExecutionPolicy Bypass -Command "$ErrorActionPreference='Stop'; $p=Start-Process -FilePath '%JAVA_CMD%' -WorkingDirectory '%SERVER_ROOT%' -ArgumentList @('-Xmx4G','-Dfile.encoding=UTF-8','-jar','.\lib\%ACTIVE_SERVER_JAR_NAME%') -RedirectStandardOutput '%LOG_DIR%\server_stdout.log' -RedirectStandardError '%LOG_DIR%\server_stderr.log' -PassThru; $p.Id | Set-Content -Encoding ascii '%LOG_DIR%\server_pid.txt'" > "%LOG_DIR%\server_start.log" 2>&1
...
powershell -NoProfile -ExecutionPolicy Bypass -Command "$ErrorActionPreference='Stop'; $p=Start-Process -FilePath '%JAVA_CMD%' -WorkingDirectory '%CLIENT_ROOT%' -ArgumentList @('-Xmx4G','-Dsun.java2d.uiScale=1.5','-Dfile.encoding=UTF-8','-jar','.\lib\%ACTIVE_CLIENT_JAR_NAME%') -RedirectStandardOutput '%LOG_DIR%\client_stdout.log' -RedirectStandardError '%LOG_DIR%\client_stderr.log' -PassThru; $p.Id | Set-Content -Encoding ascii '%LOG_DIR%\client_pid.txt'" > "%LOG_DIR%\client_start.log" 2>&1
