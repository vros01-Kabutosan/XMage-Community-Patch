...
if not defined REPO_ROOT (call :FAIL "No se encontro el clon Git autorizado." & exit /b 1)
call :LOG "Repositorio=%REPO_ROOT%"

where git.exe > "%LOG_DIR%\git_where.log" 2>&1
if errorlevel 1 (call :FAIL "Git no esta disponible en PATH." & exit /b 1)
git -C "%REPO_ROOT%" fetch origin "%BRANCH%" > "%LOG_DIR%\git_fetch.log" 2>&1
if errorlevel 1 (call :FAIL "No se pudo actualizar la rama aislada desde GitHub." & exit /b 1)
git -C "%REPO_ROOT%" worktree add --detach "%PREVIEW_REPO%" "origin/%BRANCH%" > "%LOG_DIR%\git_worktree.log" 2>&1
if errorlevel 1 (call :FAIL "No se pudo crear el worktree temporal aislado." & exit /b 1)
...
if not exist "%SOURCE_ROOT%\pom.xml" (call :FAIL "El worktree no contiene el POM RC1.3 esperado." & exit /b 1)
...
if not defined JAVA_CMD (call :FAIL "No se encontro JDK 17." & exit /b 1)
...
if errorlevel 1 (call :FAIL "Se requiere JDK 17; no se acepta Java 8 para esta compilacion." & exit /b 1)
...
if errorlevel 1 (call :FAIL "La compilacion fallo. Revisar maven_full.log." & exit /b 1)
...
if not exist "%NEW_CLIENT_JAR%" (call :FAIL "No se genero mage-client-1.4.61.jar." & exit /b 1)
if not exist "%NEW_COMMON_JAR%" (call :FAIL "No se genero mage-common-1.4.61.jar." & exit /b 1)
call :LOG "Compilacion correcta."

powershell -NoProfile -ExecutionPolicy Bypass -EncodedCommand  > "%LOG_DIR%\active_discovery.log" 2>&1
if errorlevel 1 (call :FAIL "No se localizo un cliente XMage valido en J:\mtg\xmage." & exit /b 1)
set /p "ACTIVE_CLIENT_ROOT="<"%LOG_DIR%\active_client_root.txt"
if not exist "%ACTIVE_CLIENT_ROOT%\lib" (call :FAIL "La raiz del cliente activo no es valida." & exit /b 1)
...
if !PLUGIN_RC! GEQ 8 (call :FAIL "No se pudieron copiar los plugins para la previsualizacion." & exit /b 1)
if exist "%ACTIVE_CLIENT_ROOT%\plugins\images" (
    ...
    if errorlevel 1 (call :FAIL "No se pudo enlazar el catalogo de imagenes en modo lectura." & exit /b 1)
)
...
if errorlevel 1 (call :FAIL "No se pudo iniciar el cliente de previsualizacion." & exit /b 1)
...
if errorlevel 1 (call :FAIL "El cliente no abrio una ventana XMage. Revisar client_stdout.log y client_stderr.log." & exit /b 1)
