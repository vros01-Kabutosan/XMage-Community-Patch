...
$runName = "IMPORT-CURRENT-STABLE-SOURCE-v-1.0.4-$stamp"
...
try {
    Write-Host "IMPORT CURRENT STABLE SOURCE v1.0.4"
    ...
    & robocopy $SourceRoot $stage /E /XJ /R:1 /W:1 /COPY:DAT /DCOPY:DAT /XD target .git /NFL /NDL /NP 2>&1 |
    ...
    if (Test-Path -LiteralPath $projectMavenWrapper -PathType Leaf) {
