...
$runName = "IMPORT-CURRENT-STABLE-SOURCE-v-1.0.2-$stamp"
...
try {
    Write-Host "IMPORT CURRENT STABLE SOURCE v1.0.2"
    ...
    $roboLogArg = '/LOG:"' + $runLog + '"'
    & robocopy $SourceRoot $stage /E /XJ /R:1 /W:1 /COPY:DAT /DCOPY:DAT /XD target .git $roboLogArg /NFL /NDL /NP
