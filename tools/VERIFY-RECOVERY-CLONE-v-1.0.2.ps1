...
try {
    ...
    $allowedGenerated = @($extraFiles | Where-Object { $_.Name -in @('README-RECOVERY.txt','RECOVERY-MANIFEST.tsv') })
    $unexpectedExtra = @($extraFiles | Where-Object { $_.Name -notin @('README-RECOVERY.txt','RECOVERY-MANIFEST.tsv') })
