...
Logs inside immutable backups must remain inside those backups to preserve their manifests. New operational logs belong in `<LOG_ROOT>`.

If a command cannot create its log, the operation must stop before modifying files.

### Regla estricta de logs

**Sin log en `<LOG_ROOT>`, no se ejecuta ni se acepta ninguna operación.**
