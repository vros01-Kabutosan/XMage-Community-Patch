...
public final class ManaSymbols {
    ...
    public static void loadImages() {
            ...
            try {
                ...
                for (Rarity code : codes) {
                    ...
                    if (!(MageFrame.isSkipSmallSymbolGenerationForExisting() && newFile.exists())) {// skip if option enabled and file already exists
                        ...
                        // XCP_SET_SYMBOL_GENERATION_FIX_V3_V4_R2
                        // Generate a SMALL symbol when a LARGE source exists.
                        // Only skip sets that genuinely have no source symbol.
                        if (!file.exists()) {
                            continue;
                        }
                        Image image = UI.getImageIcon(file.getAbsolutePath()).getImage();
        ...
        // XCP_SET_SYMBOL_SCAN_ALL_MISSING_R5_2G
        scanAndGenerateMissingSetSymbols();
    ...
    public static String replaceSetCodeWithHTML(String set, String rarity, int size) {
        // XCP_SET_SYMBOL_DIRECT_FALLBACK_V4_R4
        // XCP_SET_SYMBOL_AUDIT_AND_BADGE_R5_2I
        // Tooltip rule: never show an invisible/wrong empty set symbol again.
        // If an official LARGE/SMALL symbol exists, show it plus a compact code badge.
        // If it does not exist, show the code badge only and generate/audit fallback PNGs for later use.
        if (set == null || set.trim().isEmpty()) {
            return "";
        }
        set = set.trim().toUpperCase(Locale.ENGLISH);
        rarity = normalizeSetRarityCode(rarity);

        File largeFile = new File(getResourceSetsPath(ResourceSetSize.LARGE) + set + '-' + rarity + ".png");
        File smallFile = new File(getResourceSetsPath(ResourceSetSize.SMALL) + set + '-' + rarity + ".png");
        String badge = compactSetCodeFallback(set, rarity, size);

        // Missing official icon: create a clean fallback for reports/cache, but display the clear badge.
        if (!largeFile.exists() && !smallFile.exists()) {
            getOrCreateCompactSetCodeIcon(set, rarity, size, true);
            return badge;
        }
        ...
        try {
            ...
            int targetWidth = Math.max(
                    14,
                    (int) Math.round(image.getWidth() * (targetHeight / (double) image.getHeight()))
            );
            targetWidth = Math.min(42, targetWidth);

            return "<img src='"
                    + filePathToUrl(iconFile.getAbsolutePath())
                    + "' alt='"
                    + set
                    + "' height='"
                    + targetHeight
    ...
    private static void scanAndGenerateMissingSetSymbols() {
        // XCP_SET_SYMBOL_AUDIT_AND_BADGE_R5_2I
        // One automatic pass over every set known by XMage. It creates a clean
        // compact PNG for sets whose official LARGE symbol is missing and writes
        // a report next to the generated small symbols.
        try {
            File smallDir = new File(getResourceSetsPath(ResourceSetSize.SMALL));
            if (!smallDir.exists()) {
                smallDir.mkdirs();
            }
            ...
            for (String set : setCodes) {
                if (set == null || withoutSymbols.contains(set)) {
                    continue;
                }
                Set<Rarity> codes;
                if (onlyMythics.contains(set)) {
                ...
                for (Rarity rarityCode : codes) {
                    ...
                    if (!largeFile.exists()) {
                        missing++;
                        File fallback = getOrCreateCompactSetCodeIcon(set, rarity, GUISizeHelper.symbolTooltipSize, true);
            ...
            logger.warn("Symbols: R5.2i missing set symbol scan failed: " + e.getMessage());
        }
    }
    ...
    private static File getOrCreateCompactSetCodeIcon(String set, String rarity, int size) {
        return getOrCreateCompactSetCodeIcon(set, rarity, size, false);
    }

    private static File getOrCreateCompactSetCodeIcon(String set, String rarity, int size, boolean forceRewrite) {
        try {
            File dir = new File(getResourceSetsPath(ResourceSetSize.SMALL));
            ...
            try {
                ...
                Color color = getCompactFallbackColor(rarity);
                g.setColor(new Color(255, 255, 255, 230));
                g.fillRoundRect(0, 0, width - 1, height - 1, 6, 6);
                g.setColor(color);
                g.drawRoundRect(0, 0, width - 1, height - 1, 6, 6);

                Font font = new Font(Font.SANS_SERIF, Font.BOLD, Math.max(10, height - 6));
                g.setFont(font);
    ...
    private static String compactSetCodeFallback(String set, String rarity, int size) {
        ...
        if ("M".equalsIgnoreCase(rarity)) {
            color = "#D5330B";
        } else if ("R".equalsIgnoreCase(rarity)) {
            color = "#B8860B";
        } else if ("U".equalsIgnoreCase(rarity)) {
            color = "#666666";
        } else {
            color = "#111111";
        }
