...
public final class GuiDisplayUtil {
    ...
    public static StringBuilder getRulesFromCardView(CardView card, TextLines textLines) {
        ...
        // XCP_TOOLTIP_NO_CLIP_SET_BADGE_R5_2I
        // Fixed-width HTML + real preferred-height measurement in CardInfoPane.
        // Slightly larger than R5.2e/f, without the huge blank bottom.
        ...
        if (card.isSplitCard()) {
            ...
            for (String ruling : card.getLeftSplitRules()) {
                if (ruling != null && !ruling.replace(".", "").trim().isEmpty()) {
                    ...
                    rule.append("<p style='margin:2px 2px 2px 2px'>").append(replaceNamesInRule(ruling, card.getLeftSplitName())).append("</p>");
                }
            }
            rule.append("<table cellspacing=0 cellpadding=0 border=0 width='100%'>");
            rule.append("<tr><td valign='top'><b>");
            ...
            for (String ruling : card.getRightSplitRules()) {
                if (ruling != null && !ruling.replace(".", "").trim().isEmpty()) {
                    // split names must be replaced
                    duplicatedRules.add(ruling);
                    rule.append("<p style='margin:2px 2px 2px 2px'>").append(replaceNamesInRule(ruling, card.getRightSplitName())).append("</p>");
                }
            }
        }
        ...
        if (zone != null) {
            buffer.append("<p style='margin:2px 2px 2px 2px'><b>Card Zone:</b> ").append(zone).append("</p>");
        }
