...
public final class GamePanel extends javax.swing.JPanel {
    ...
    public GamePanel() {
        ...
        // XCP_BOARD_LAYOUT_POLISH_V3_V4
        // The fixed stack is gone, so do not waste ~30% of the command area on
        // an empty split pane. Give the hand the full width and place the
        // shortcut controls in a compact strip below it.
        JPanel pnlCommandsSkipAndStack = new JPanel(new BorderLayout());
        pnlCommandsSkipAndStack.setOpaque(false);
        pnlCommandsSkipAndStack.add(pnlShortCuts, BorderLayout.SOUTH);
        ...
        pnlCommandsRoot.add(pnlCommandsSkipAndStack, BorderLayout.SOUTH);
        ...
        // XCP shortcut order: turn flow first, then control actions.
    ...
    private void setGUISize(boolean themeReload) {
        ...
        pnlShortCuts.setPreferredSize(new Dimension(Short.MAX_VALUE, upperPanelsHeight));
        pnlShortCuts.setMaximumSize(new Dimension(Short.MAX_VALUE, upperPanelsHeight));

        // XCP V3/V4 floating stack card scale
        stackObjects.setCardDimension(getFloatingStackCardDimension());
        ...
        pnlShortCuts.setLayout(new FlowLayout(FlowLayout.CENTER, hGap, vGap));
        // skip buttons - sizes
    ...
    public synchronized void updateGame() {
        if (playerId == null && lastGameData.game.getWatchedHands().isEmpty()) {
            ...
            // XCP_PERSISTENT_HAND_SHIELD_HOOK_V1 - isolated read-only adapter
    ...
    // XCP_FLOATING_STACK_POLISH_V3_V4_START
    private Dimension getFloatingStackCardDimension() {
        Dimension base = GUISizeHelper.handCardDimension;
        double scale = 1.08;
        return new Dimension(
                Math.max(base.width, (int) Math.round(base.width * scale)),
                Math.max(base.height, (int) Math.round(base.height * scale))
        );
    }

    private java.util.prefs.Preferences getFloatingStackPreferences() {
        return java.util.prefs.Preferences
                .userNodeForPackage(GamePanel.class)
                .node(XCP_STACK_PREF_NODE);
    }

    private void restoreFloatingStackBounds() {
        if (floatingStackFrame == null) {
            return;
        }

        JDesktopPane desktop = MageFrame.getDesktop();
        Dimension desktopSize = desktop == null
                ? new Dimension(1280, 720)
                : desktop.getSize();

        Dimension cardSize = getFloatingStackCardDimension();
        int defaultWidth = Math.max(290, cardSize.width + 90);
        int defaultHeight = Math.max(500, cardSize.height * 2 + 90);

        java.util.prefs.Preferences prefs = getFloatingStackPreferences();

        int width = Math.max(
                floatingStackFrame.getMinimumSize().width,
                prefs.getInt(XCP_STACK_W, defaultWidth)
        );
        int height = Math.max(
                floatingStackFrame.getMinimumSize().height,
                prefs.getInt(XCP_STACK_H, defaultHeight)
        );
        ...
        int defaultY = Math.max(10, (desktopSize.height - height) / 2);
        ...
        floatingStackRestoringBounds = true;
        floatingStackFrame.setBounds(x, y, width, height);
        floatingStackRestoringBounds = false;
    }

    private void saveFloatingStackBounds() {
        if (floatingStackFrame == null || floatingStackRestoringBounds) {
            return;
        }

        Rectangle bounds = floatingStackFrame.getBounds();
    ...
    private JPanel createFloatingStackHeader() {
        ...
        header.setBackground(new Color(28, 29, 36));
        header.setBorder(new EmptyBorder(7, 10, 7, 10));

        floatingStackTitleLabel = new JLabel("Spells in stack: 0");
        floatingStackTitleLabel.setFont(
                GUISizeHelper.dialogFont.deriveFont(Font.BOLD)
        );
        floatingStackTitleLabel.setForeground(new Color(235, 235, 240));
        header.add(floatingStackTitleLabel, BorderLayout.WEST);

        JLabel dragHint = new JLabel("");
        dragHint.setFont(GUISizeHelper.dialogFont.deriveFont(Font.PLAIN, Math.max(9f, GUISizeHelper.dialogFont.getSize2D() - 2f)));
        dragHint.setForeground(new Color(150, 152, 162));
        header.add(dragHint, BorderLayout.EAST);

        MouseAdapter dragAdapter = new MouseAdapter() {
            @Override
            public void mousePressed(MouseEvent e) {
                floatingStackDragOrigin = e.getPoint();
            }

            @Override
            public void mouseDragged(MouseEvent e) {
                if (floatingStackFrame == null || floatingStackDragOrigin == null) {
                    return;
                }

                Point frameLocation = floatingStackFrame.getLocation();
                int nextX = frameLocation.x + e.getX() - floatingStackDragOrigin.x;
                int nextY = frameLocation.y + e.getY() - floatingStackDragOrigin.y;

                JDesktopPane desktop = MageFrame.getDesktop();
                if (desktop != null) {
                    nextX = Math.max(
                            0,
                            Math.min(nextX, Math.max(0, desktop.getWidth() - floatingStackFrame.getWidth()))
                    );
                    nextY = Math.max(
                            0,
                            Math.min(nextY, Math.max(0, desktop.getHeight() - floatingStackFrame.getHeight()))
                    );
                }

                floatingStackFrame.setLocation(nextX, nextY);
            }

            @Override
            public void mouseReleased(MouseEvent e) {
                saveFloatingStackBounds();
                floatingStackDragOrigin = null;
            }
        };
    ...
    // XCP_STACK_ABILITY_CONTEXT_V1_0_1_START
    /**
     * Presents the exact ability text received for the next stack object.
     * StackAbilityView already contains selected modes, targets and hints;
     * this client-only panel makes that information readable without changing
     * game or server logic.
     */
    private void updateFloatingStackAbilityContext(CardView stackCard) {
        if (floatingStackAbilityPanel == null || floatingStackAbilityText == null) {
            return;
        }

        if (!(stackCard instanceof StackAbilityView)) {
            floatingStackAbilityPanel.setVisible(false);
            floatingStackAbilityText.setText("");
            return;
        }

        StackAbilityView abilityView = (StackAbilityView) stackCard;
        CardView sourceCard = abilityView.getSourceCard();
        String sourceName = sourceCard == null || sourceCard.getName() == null
                ? "Ability"
                : sourceCard.getName();

        StringBuilder body = new StringBuilder(256);
        body.append("<html><body style='margin:4px; font-family:sans-serif;'>")
                .append("<b>").append(sourceName).append("</b><br>");
        List<String> rules = abilityView.getRules();
        if (rules != null && !rules.isEmpty()) {
            for (String rule : rules) {
                if (rule != null && !rule.trim().isEmpty()) {
                    body.append(rule).append("<br>");
                }
            }
        }
        body.append("</body></html>");

        floatingStackAbilityText.setText(body.toString());
        floatingStackAbilityText.setCaretPosition(0);
        floatingStackAbilityPanel.setVisible(true);
        floatingStackAbilityPanel.revalidate();
        floatingStackAbilityPanel.repaint();
    }
    // XCP_STACK_ABILITY_CONTEXT_V1_0_1_END
    ...
    private void initFloatingStackWindow() {
        ...
        stackObjects.setVerticalStackLayout(true);
        stackObjects.setCardDimension(getFloatingStackCardDimension());
        stackObjects.setBackgroundColor(new Color(18, 19, 24));
        stackObjects.setBorder(new EmptyBorder(8, 8, 8, 8));

        floatingStackFrame = new JInternalFrame(
                "",
                true,
                false,
                false,
        ...
        floatingStackFrame.setDefaultCloseOperation(WindowConstants.DO_NOTHING_ON_CLOSE);
        floatingStackFrame.setFrameIcon(null);
        floatingStackFrame.setOpaque(true);
        floatingStackFrame.setBackground(new Color(18, 19, 24));
        floatingStackFrame.setBorder(
                BorderFactory.createCompoundBorder(
                        new LineBorder(new Color(78, 80, 92), 1, true),
                        new EmptyBorder(1, 1, 1, 1)
                )
        );

        if (floatingStackFrame.getUI() instanceof javax.swing.plaf.basic.BasicInternalFrameUI) {
            ((javax.swing.plaf.basic.BasicInternalFrameUI) floatingStackFrame.getUI()).setNorthPane(null);
        }

        JPanel content = new JPanel(new BorderLayout());
        content.setOpaque(true);
        content.setBackground(new Color(18, 19, 24));

        floatingStackHeader = createFloatingStackHeader();
        content.add(floatingStackHeader, BorderLayout.NORTH);

        // XCP_STACK_ABILITY_CONTEXT_V1_0_1: compact readable context for the
        // currently resolving ability; hidden for ordinary spells.
        floatingStackAbilityPanel = new JPanel(new BorderLayout(4, 2));
        floatingStackAbilityPanel.setOpaque(true);
        floatingStackAbilityPanel.setBackground(new Color(27, 29, 37));
        floatingStackAbilityPanel.setBorder(new EmptyBorder(5, 8, 5, 8));

        JLabel abilityCaption = new JLabel("Current ability");
        abilityCaption.setForeground(new Color(190, 220, 195));
        abilityCaption.setFont(GUISizeHelper.dialogFont.deriveFont(Font.BOLD));
        floatingStackAbilityPanel.add(abilityCaption, BorderLayout.NORTH);

        floatingStackAbilityText = new JEditorPane("text/html", "");
        floatingStackAbilityText.setEditable(false);
        floatingStackAbilityText.setOpaque(true);
        floatingStackAbilityText.setBackground(new Color(27, 29, 37));
        floatingStackAbilityText.setForeground(new Color(235, 235, 240));
        floatingStackAbilityText.setFont(GUISizeHelper.dialogFont);
        floatingStackAbilityText.setBorder(null);

        JScrollPane abilityScroll = new JScrollPane(floatingStackAbilityText);
        abilityScroll.setBorder(BorderFactory.createLineBorder(new Color(70, 74, 88)));
        abilityScroll.setOpaque(false);
        abilityScroll.getViewport().setOpaque(true);
        abilityScroll.getViewport().setBackground(new Color(27, 29, 37));
        abilityScroll.setHorizontalScrollBarPolicy(ScrollPaneConstants.HORIZONTAL_SCROLLBAR_NEVER);
        abilityScroll.setVerticalScrollBarPolicy(ScrollPaneConstants.VERTICAL_SCROLLBAR_AS_NEEDED);
        abilityScroll.setPreferredSize(new Dimension(260, 92));
        floatingStackAbilityPanel.add(abilityScroll, BorderLayout.CENTER);
        floatingStackAbilityPanel.setVisible(false);

        JPanel stackCenter = new JPanel(new BorderLayout());
        stackCenter.setOpaque(false);
        stackCenter.add(floatingStackAbilityPanel, BorderLayout.NORTH);
        stackCenter.add(stackObjects, BorderLayout.CENTER);
        content.add(stackCenter, BorderLayout.CENTER);

        // XCP_STACK_BOTTOM_RESIZE_V4_R4
        JPanel bottomResizeGrip = new JPanel(new BorderLayout());
        bottomResizeGrip.setOpaque(true);
        bottomResizeGrip.setBackground(new Color(35, 36, 43));
        bottomResizeGrip.setPreferredSize(new Dimension(1, 12));
        bottomResizeGrip.setCursor(Cursor.getPredefinedCursor(Cursor.S_RESIZE_CURSOR));
        bottomResizeGrip.setToolTipText("Drag to resize stack height");

        JLabel resizeMark = new JLabel("ÃƒÂ¢Ã¢â‚¬Â¢Ã‚Â", SwingConstants.CENTER);
        resizeMark.setForeground(new Color(90, 92, 103));
        resizeMark.setFont(GUISizeHelper.dialogFont.deriveFont(Font.PLAIN, 9f));
        bottomResizeGrip.add(resizeMark, BorderLayout.CENTER);
        ...
        MouseAdapter bottomResizeAdapter = new MouseAdapter() {
            ...
            public void mousePressed(MouseEvent e) {
                ...
                floatingStackResizeOrigin = e.getLocationOnScreen();
                floatingStackResizeStartSize = floatingStackFrame.getSize();
            }

            @Override
            public void mouseDragged(MouseEvent e) {
                if (floatingStackFrame == null
                        || floatingStackResizeOrigin == null
                        || floatingStackResizeStartSize == null) {
                    return;
                }

                Point now = e.getLocationOnScreen();
                int dy = now.y - floatingStackResizeOrigin.y;
                int minHeight = floatingStackFrame.getMinimumSize().height;
                int newHeight = Math.max(minHeight, floatingStackResizeStartSize.height + dy);

                JDesktopPane desktop = MageFrame.getDesktop();
                if (desktop != null) {
                    newHeight = Math.min(
                            newHeight,
                            Math.max(minHeight, desktop.getHeight() - floatingStackFrame.getY())
                    );
                }

                floatingStackFrame.setSize(floatingStackFrame.getWidth(), newHeight);
                floatingStackFrame.revalidate();
                floatingStackFrame.repaint();
            }
        ...
        resizeMark.addMouseListener(bottomResizeAdapter);
        resizeMark.addMouseMotionListener(bottomResizeAdapter);
        content.add(bottomResizeGrip, BorderLayout.SOUTH);

        floatingStackFrame.setContentPane(content);

        Dimension cardSize = getFloatingStackCardDimension();
        floatingStackFrame.setMinimumSize(
                new Dimension(
                        Math.max(235, cardSize.width + 50),
                        Math.max(330, cardSize.height + 120)
                )
        );

        JDesktopPane desktop = MageFrame.getDesktop();
        if (desktop != null) {
            desktop.add(floatingStackFrame, JLayeredPane.PALETTE_LAYER);
        }

        restoreFloatingStackBounds();

        floatingStackFrame.addComponentListener(new ComponentAdapter() {
            @Override
        ...
        floatingStackFrame.setVisible(false);
    }
    ...
    private void disposeFloatingStackWindow() {
        ...
        Container parent = floatingStackFrame.getParent();
        if (parent != null) {
            parent.remove(floatingStackFrame);
            parent.revalidate();
            parent.repaint();
        }

        floatingStackFrame.dispose();
        floatingStackFrame = null;
        floatingStackHeader = null;
        floatingStackTitleLabel = null;
        floatingStackAbilityPanel = null;
        floatingStackAbilityText = null;
        floatingStackDragOrigin = null;
        floatingStackResizeOrigin = null;
        floatingStackResizeStartSize = null;
        floatingStackHadObjects = false;
    }

    private void updateFloatingStackVisibility(int objectCount) {
        if (floatingStackFrame == null) {
            initFloatingStackWindow();
        }

        boolean hasObjects = objectCount > 0;

        if (floatingStackFrame != null) {
            if (floatingStackTitleLabel != null) {
                // XCP_STACK_HEADER_R5_1C
                floatingStackTitleLabel.setText("Spells in stack: " + objectCount);
            }

            if (hasObjects && !floatingStackHadObjects) {
                floatingStackFrame.setVisible(true);
                floatingStackFrame.toFront();
            } else if (!hasObjects && floatingStackHadObjects) {
                saveFloatingStackBounds();
                floatingStackFrame.setVisible(false);
            }
        }

        floatingStackHadObjects = hasObjects;
    }

    private void displayStack(GameView game, BigCard bigCard, FeedbackPanel feedbackPanel, UUID gameId) {
        // XCP_STACK_ORDER_R5_1C
        // Server stack is already LIFO: newest / next-to-resolve first.
        CardView nextStackObject = game.getStack().isEmpty()
                ? null
                : game.getStack().values().iterator().next();
        this.stackObjects.loadCards(game.getStack(), bigCard, gameId, false);
        updateFloatingStackAbilityContext(nextStackObject);
        updateFloatingStackVisibility(game.getStack().size());
    }
    // XCP_FLOATING_STACK_POLISH_V3_V4_END
    ...
    private void initComponents() {
        ...
        // XCP V3/V4: splitHandAndStack kept for plugin compatibility, but no longer participates in visible layout or persisted divider state.
        ...
        // XCP_BOARD_LAYOUT_POLISH_V3_V4_PHASES
        // Keep the familiar vertical phase rail, but anchor it to the bottom
        // instead of wasting the upper-right battlefield area.
        phasesContainer = new JPanel(new BorderLayout());
        phasesContainer.setOpaque(false);
        phasesContainer.setBorder(new EmptyBorder(0, 2, 8, 2));
        phasesContainer.add(jPhases, BorderLayout.SOUTH);
    ...
    // XCP_FLOATING_STACK_POLISH_V3_V4_FIELDS
    private JInternalFrame floatingStackFrame;
    private JLabel floatingStackTitleLabel;
    private JPanel floatingStackHeader;
    // XCP_STACK_ABILITY_CONTEXT_V1_0_1
    private JPanel floatingStackAbilityPanel;
    private JEditorPane floatingStackAbilityText;
    ...
    private static final String XCP_STACK_PREF_NODE = "xcpFloatingStackV4";
